...
describe('AudioBuffer', () => {
  ...
  describe('Frame Transmission', () => {
    ...
    it('should send frames with correct Twilio format', () => {
      ...
      const sentData = JSON.parse((mockWebSocket.send as jest.Mock).mock.calls[0][0] as string);
  ...
  describe('Stop and Flush', () => {
    ...
    it('should flush remaining audio data', () => {
      ...
      const mediaCalls = calls.filter(call => JSON.parse(call[0] as string).event === 'media');
      const markCalls = calls.filter(call => JSON.parse(call[0] as string).event === 'mark');
    ...
    it('should pad partial frames with silence during flush', () => {
      ...
      const mediaCalls = (mockWebSocket.send as jest.Mock).mock.calls
        .filter(call => JSON.parse(call[0] as string).event === 'media');
      expect(mediaCalls.length).toBe(1);

      const sentData = JSON.parse(mediaCalls[0][0] as string);
